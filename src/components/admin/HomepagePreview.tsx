...
const HomepagePreview = ({ settings }: HomepagePreviewProps) => {
  return (
    <div className="w-full max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      {/* CTA Section Preview */}
      <div className="p-8 bg-gradient-to-br from-primary via-primary/90 to-accent text-white">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          <div>
            <h2 className={`${settings.section_title_size || 'text-4xl'} font-bold mb-4`}>
              {settings.cta_title || "Sẵn sàng trải nghiệm Nova Souls?"}
            </h2>
            <p className="text-white/90 mb-6">
              {settings.cta_subtitle || "Đặt món ngay hôm nay và khám phá hương vị cà phê tuyệt vời..."}
            </p>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-xs text-white/80">{settings.cta_address || "123 Đường ABC, Quận 1, TP.HCM"}</div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-xs text-white/80">{settings.cta_phone || "0123 456 789"}</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-4">
              <Button className="bg-accent text-white">
                {settings.cta_button_primary_text || "Đặt món ngay"}
              </Button>
              <Button variant="outline" className="text-white border-white/30">
                {settings.cta_button_secondary_text || "Liên hệ"}
              </Button>
            </div>
          </div>
            ...
            <CardContent className="p-6">
              <div className="text-center mb-4">
                ...
                <h3 className="text-lg font-bold mb-2">
                  {settings.cta_newsletter_title || "Nhận ưu đãi đặc biệt"}
                </h3>
                <p className="text-white/80 text-sm">
                  {settings.cta_newsletter_description || "Đăng ký nhận tin để nhận thông tin về ưu đãi..."}
                </p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  <input 
                    ...
                    className="flex-1 px-3 py-2 bg-white/20 border border-white/30 rounded text-white placeholder:text-white/60 text-sm"
                  />
                  <Button size="sm" className="bg-accent text-white">Đăng ký</Button>
                </div>
                
                <div className="space-y-2 text-xs text-white/80">
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-accent-light rounded-full"></div>
                    <span>{settings.cta_newsletter_benefit_1 || "Giảm 10% cho đơn hàng đầu tiên"}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-accent-light rounded-full"></div>
                    <span>{settings.cta_newsletter_benefit_2 || "Thông báo sớm về sản phẩm mới"}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-accent-light rounded-full"></div>
                    <span>{settings.cta_newsletter_benefit_3 || "Ưu đãi đặc biệt cho thành viên"}</span>
                  </div>
                </div>
              </div>
            </CardContent>
